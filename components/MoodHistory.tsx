...
import { SavedMood, Language } from '../types';
...
const MoodHistory: React.FC<Props> = ({ history, language }) => {
            ...
            <div key={i} className="bg-white/5 p-5 md:p-8 rounded-2xl md:rounded-[2rem] flex justify-between items-center text-sm md:text-lg border border-white/5 hover:border-red-600/30 transition-all group">
              <div className={`${isRtl ? 'text-right' : 'text-left'} space-y-1`}>
                ...
                <p className="font-black">
                  {t.moods[item.mood as keyof typeof t.moods] || item.mood} 
                  <span className="mx-2 opacity-30">/</span>
                  <span className="text-red-600 opacity-80">{t.intensityLevels[item.intensity as keyof typeof t.intensityLevels] || item.intensity}</span>
                </p>
              </div>
