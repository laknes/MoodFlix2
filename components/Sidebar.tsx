...
const Sidebar: React.FC<Props> = ({ theme, language, user, onThemeToggle, onLangToggle, activeView, onViewChange }) => {
  ...
  const HistoryIcon = <svg className="w-5 h-5 md:w-6 md:h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>;
  ...
  return (
    <>
      {/* Mobile Bottom Bar - Hidden on large screens */}
      ...
      >
        <div className="p-8 flex items-center justify-center border-b border-white/5 mb-6">
          <h2 className={`netflix-logo netflix-red transition-all duration-500 font-black ${isOpen ? 'text-3xl' : 'text-xl'}`}>
        ...
        <div className="p-6 border-t border-white/5 space-y-6">
          {isOpen ? (
            <div className="space-y-6 animate-fade-in">
              <div className="flex flex-col gap-2">
                ...
                <button 
                  ...
                  className="w-full bg-white/5 p-3 rounded-xl flex items-center justify-between hover:bg-white/10 transition-colors border border-white/5"
                >
                  <span className="text-sm font-bold">{theme === 'dark' ? t.darkMode : t.lightMode}</span>
                  <span className="text-lg">{theme === 'dark' ? MoonIcon : SunIcon}</span>
                </button>
              </div>
              <div className="flex flex-col gap-2">
                ...
                <div className="flex gap-2">
                  <button 
                    ...
                    className={`flex-1 p-2 rounded-lg text-[10px] font-black transition-all border ${language === 'fa' ? 'bg-red-600 border-red-600 text-white' : 'bg-white/5 border-transparent hover:bg-white/10'}`}
                  >
                    فارسی
                  </button>
                  <button 
                    onClick={() => onLangToggle('en')}
                    className={`flex-1 p-2 rounded-lg text-[10px] font-black transition-all border ${language === 'en' ? 'bg-red-600 border-red-600 text-white' : 'bg-white/5 border-transparent hover:bg-white/10'}`}
                  >
                    EN
                  </button>
                </div>
            ...
            <div className="flex flex-col items-center gap-6 py-2">
              <button onClick={onThemeToggle} className="text-slate-400 hover:text-red-500 hover:scale-110 transition-all">
