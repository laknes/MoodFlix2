...
const Profile: React.FC<Props> = ({ user, language, onUpdate, onLogout }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto py-8 md:py-16 px-4 animate-fade-in">
      <div className="glass-card p-8 md:p-12 rounded-[2.5rem] md:rounded-[3rem] relative overflow-hidden border border-white/10 dark:border-white/5">
        ...
        <form onSubmit={handleSubmit} className="space-y-8 relative z-10">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <input 
                type="text" 
                className="w-full bg-white/5 border border-white/10 rounded-xl px-6 py-4 focus:border-red-600 outline-none transition-all font-bold text-black dark:text-white"
                value={formData.name}
                onChange={e => setFormData({ ...formData, name: e.target.value })}
              />
            </div>
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest px-1">{t.birthday}</label>
              <input 
                type="date" 
                className="w-full bg-white/5 border border-white/10 rounded-xl px-6 py-4 focus:border-red-600 outline-none transition-all font-bold text-black dark:text-white"
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-full text-xs font-black transition-all border ${
                    formData.favoriteGenres.includes(genre)
                      ...
                      : 'bg-white/5 border-white/10 text-slate-500 dark:text-slate-400 hover:border-red-600/30'
                  }`}
                >
                  {genre}
                </button>
          ...
          <div className="space-y-2">
            ...
            <textarea 
              className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 focus:border-red-600 outline-none transition-all font-bold min-h-[100px] text-black dark:text-white"
